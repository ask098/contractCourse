
import * as constants from '@/store/constants'
import CertifierContract from '@/contracts/Certifier.json'
const state = {
  provider: !!(window.web3 && window.web3.currentProvider),
  isOwner: false,
  name: null,
  contract: null,
  coinbase: null,
  cursos: null,
}

const contractHash = '0x3f37d764c99e57b291cf8c932b99d0e8f6c18cde'

const actions = {
  [constants.CERTIFIER_INIT]: ({commit}) => {
    const abi = CertifierContract.abi
    const contractAddress = contractHash
    const instance = web3.eth.contract(abi).at(contractAddress)
    commit(constants.CERTIFIER_SET_INSTANCE, instance)
    web3.eth.getCoinbase((error, coinbase) => {
      if(error) console.error(error)
      commit(constants.CERTIFIER_SET_COINBASE, coinbase)
      instance.isOwner({from: coinbase}, (error, isOwner) => {
        if(error) console.error(error)
        commit(constants.CERTIFIER_SET_IS_OWNER, isOwner)
      })
      instance.getName({from:coinbase},(error,name)=>{
        if(error) console.error(error)
        commit(constants.CERTIFIER_SET_NAME,name)
      })
      instance.getCourses({from:coinbase},(error,cursos) => {
        if(error) console.error(error)
        console.log(cursos)
        commit(constants.CERTIFIER_SET_COURSES, cursos)
      })
    })
  },
  [constants.CERTIFIER_SET_NAME_BY_OWNER]: ({commit, state}, data) => {
    const abi = CertifierContract.abi
    const contractAddress = contractHash
    const instance = web3.eth.contract(abi).at(contractAddress)
    instance.setName(data, {from:state.coinbase},(error,tx)=>{
        if(error) console.error(error)
        console.log(tx);
    })
  },
  [constants.CERTIFIER_SET_NEW_COURSE_BY_OWNER]: ({commit, state}, data) => {
    const abi = CertifierContract.abi
    const contractAddress = contractHash
    const instance = web3.eth.contract(abi).at(contractAddress)
    const {newCourseCode, newCourseName, newCourseCost, newCourseDuration, newCourseThreshold, newCourseCodes} = data
    instance.addCourse(newCourseCode, newCourseName, newCourseCost, newCourseDuration, newCourseThreshold, [newCourseCodes], {from:state.coinbase},(error,tx)=>{
        if(error) console.error(error)
        console.log(tx);
        console.log("Curso creado");
    })
  },
  [constants.CERTIFIER_SET_NEW_STUDENT_COURSE]: ({commit, state}, data) => {
    const abi = CertifierContract.abi
    const contractAddress = contractHash
    const instance = web3.eth.contract(abi).at(contractAddress)

    const {courseCode, firstName, lastName, age, email} = data
    instance.subscribe(courseCode, firstName, lastName, age, email, {from:state.coinbase, value: web3.toWei(3, 'ether')},(error,tx)=>{
        if(error) console.error(error)
        console.log(tx);
        console.log("Estudiante creado");
    })
  }
}

const mutations = {
  [constants.CERTIFIER_SET_INSTANCE]: (state, instance) => {
    state.contract = instance
  },
  [constants.CERTIFIER_SET_COINBASE]: (state, coinbase) => {
    state.coinbase = coinbase
  },
  [constants.CERTIFIER_SET_IS_OWNER]: (state, isOwner) => {
    state.isOwner = isOwner
  },
  [constants.CERTIFIER_SET_NAME]:(state,name)=>{
    state.name= name
  },
  [constants.CERTIFIER_SET_COURSES]:(state,cursos)=>{
    state.cursos=cursos
  }
}

const getters = {}

export default {
  state,
  actions,
  mutations,
  getters
}
